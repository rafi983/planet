...
export default function PlanetNavigationComponent({
    ...
    <div
      ...
      className={`planetNavItem font-spartan text-[9px] text-white tracking-[1.93px] uppercase  h-full flex items-center  
        ...
        md:w-[281px] md:h-[40px] md:border-1 md:border-[#979797] md:text-[9px] ${
          pageSection == item && colors.hasOwnProperty(planet)
            ? `${colors[planet][0]} ${colors[planet][1]}`
            : "border-transparent "
        } md:hover:bg-white md:hover:bg-opacity-50
        xl:w-full xl:text-[18px]  xl:h-[48px] md:px-5 cursor-pointer`}
      onClick={() => {
        setPageSection(item);
      }}
    >
      <span className="hidden md:block md:opacity-50 xl:mr-5">
        0{key + 1}&nbsp;
      </span>{" "}
      {item}
    </div>
  ...
  return (
    <div
      className="flex md:flex-col justify-around items-center h-[50px]  border border-x-0 border-y-1 border-[#979797] border-opacity-35 z-19 
