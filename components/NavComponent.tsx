...
export default function NavComponent() {
  ...
  const navItems = Object.keys(planetData).map((key) => {
    const colors = {
      ...
      jupiter: [
        `border-planetColors-jupiter`,
        `bg-planetcolors-jupiter`,
        `#D83A34`,
      ],
      saturn: [
        `border-planetColors-saturn`,
        `bg-planetcolors-saturn`,
        `#CD5120`,
      ],
      neptune: [
        `border-planetColors-neptune`,
        `bg-planetcolors-neptune`,
        `#1EC1A2`,
      ],
      uranus: [
        `border-planetColors-neptune`,
      ...
      mercury: [
        `border-planetColors-mercury`,
        `bg-planetcolors-mercury`,
        `#419EBB`,
      ],
    } as PlanetColors;
    ...
    return (
      <Link
        ...
        className={`uppercase cursor-pointer flex items-center w-full h-[65px] mx-auto 
          border border-x-0 border-t-0 border-b-1  border-opacity-50 ${
            colors[key.toLowerCase()][0]
          }
      ...
      >
        ...
        xl:h-full hover:xl:border-t-4 hover:md:opacity-100 ${
          colors[key.toLowerCase()][0]
        } xl:flex xl:justify-center xl:items-center xl:w-full ${
            key == activeNav ? `md:opacity-[100%]` : ""
          }`}
        >
          {key}
        </div>
