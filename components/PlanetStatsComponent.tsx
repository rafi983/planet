...
export default function PlanetStatsComponent({
  ...
  const statItem = statItems.map((item, key) => (
    <div
      key={key}
      className=" flex justify-between items-center uppercase font-spartan font-bold text-[8px] leading-[16px] tracking-[0.73px] 
    ...
    >
      <span style={{ opacity: "50%" }} className="">
        {item[0]}
      </span>
  ...
  return (
    <div
      className="text-white w-[90%] h-[216px] mx-auto flex flex-col justify-around
    md:flex-row md:row-start-10 md:col-start-2 md:col-span-10 md:row-span-1 md:h-[88px] md:w-full xl:row-span-2 xl:row-start-10"
    >
      {statItem}
    </div>
  );
